...
class ClassesSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // ابتدائي: 1-6
        for ($i = 1; $i <= 6; $i++) {
            SchoolClass::create(['name' => "الصف $i ابتدائي", 'grade_order' => $i, 'stage_id' => $primary->id]);
        }

        // إعدادي: 7-9
        for ($i = 7; $i <= 9; $i++) {
            SchoolClass::create(['name' => "الصف $i إعدادي", 'grade_order' => $i, 'stage_id' => $middle->id]);
        }
        ...
        foreach ($highNames as $order => $name) {
            SchoolClass::create(['name' => $name, 'grade_order' => $order, 'stage_id' => $scientific->id]);
            SchoolClass::create(['name' => $name, 'grade_order' => $order, 'stage_id' => $literary->id]);
        }
    }
}
